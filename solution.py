...
def filter_by_train_and_country(input_trains, train_name, input_country_name):
    train_df = input_trains[train_name]
    filtered_df = train_df[train_df['Area'] == input_country_name]
...
filtered_areas = ['China, Macao SAR', "Ethiopia PDR", "Niue", "Tuvalu", "USSR", "Vanuatu"]
unique_areas = [area for area in unique_areas if area not in filtered_areas]
...
for area in unique_areas:
    ...
    pivot_df_food_trade = filtered_df_food_trade_exported_value.pivot(index='Year Code', columns='Item Code (CPC)', values='Value')
    ...
    pivot_df_crops_production = filtered_df_crops_production.pivot(index='Year Code', columns='Item Code (CPC)', values='Value')
    ...
    pivot_df_employment = filtered_df_employment_indicator_code_21144.pivot(index='Year Code', columns='Indicator Code', values='Value')
    ...
    existing_columns_food_balance = [col for col in food_balance_unique_item_code_list if
                                     col in pivot_df_food_trade.columns]
    pivot_df_food_balance = filtered_df_food_balance_exported_quantity.pivot(index='Year Code', columns='Item Code (FBS)', values='Value')
    ...
    pivot_df_food_security = filtered_df_food_security_by_unique_item_code.pivot(index='Year Code', columns='Item Code', values='Value')
    ...
    pivot_df_foreign_direct_investment = filtered_df_foreign_direct_investment_23080.pivot(index='Year Code', columns='Item Code', values='Value')
    ...
    pivot_df_land_temperature_change = filtered_df_land_temperature_change_by_codes.pivot(index='Year Code', columns='Element Code', values='Value')
    ...
    # plt.show() # NOTE: Commented out to avoid blocking the execution. It's working well on Spyder, but not on Intellij
...
result_file_name = "934G5-Solution-276284.csv"
